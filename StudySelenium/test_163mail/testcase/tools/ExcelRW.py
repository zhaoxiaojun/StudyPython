    ...
    def readcell(self,sheetname='sheet1',rown=1,coln=1):
        ...
        try:
            if self.isopentrue == True:
                ...
                print('[file:%s,sheet:%s,row:%s,col:%s]:%s.'%(self.xls_name,sheetname,rown+1,coln+1,cell))
                return cell
            else:
                print('file %s is not open.'%self.xls_name)
                return -2
    ...
    def readrow(self,sheetname='sheet1',rown=1):
        ...
        try:
            if self.isopentrue == True:
                ...
                print('[file:%s,sheet:%s,row:%s]:%s.'%(self.xls_name,sheetname,rown+1,row))
                return row
            else:
                print('file %s is not open.'%self.xls_name)
                return -2
    ...
    def readcol(self,sheetname='sheet1',coln=1):
        ...
        try:
            if self.isopentrue == True:
                ...
                print('[file:%s,sheet:%s,col:%s]:%s.'%(self.xls_name,sheetname,coln+1,col))
                return col
            else:
                print('file %s is not open.'%self.xls_name)
                return -2
    ...
    def writecell(self,value='',sheetn=1,rown=1,coln=1):
        ...
        try:
            if self.isopentrue == True:
                ...
                print('writecell value:%s to [sheet:%s,row:%s,col:%s] is ture.'%(value,sheetn,rown+1,coln+1))
                return 0
            else:
                print('file %s is not open.'%self.xls_name)
                return -2
        ...
        try:
            if self.isopentrue == True:
                ...
                for value in values:
                    ...
                    coln += 1
                xlrd_objectc.save(self.xls_name)
                print('writerow values:%s to [sheet:%s,row:%s,col:%s] is ture.'%(values,sheetn,rown+1,coln+1))
                return 0
            else:
                print('file %s is not open.'%self.xls_name)
                return -2
        ...
        try:
            if self.isopentrue == True:
                ...
                for value in values:
                    ...
                    rown += 1
                xlrd_objectc.save(self.xls_name)
                print('writecol values:%s to [sheet:%s,row:%s,col:%s] is ture.'%(values,sheetn,rown+1,coln+1))
                return 0
            else:
                print('file %s is not open.'%self.xls_name)
                return -2
...
if __name__ == '__main__': 
    #初始化对象
    xlseng = XlsEngine('E:\\Code\\yzhselenium\\test_163mail\\data\\test1.xls')
    ...
    #读取sheet1页第3行第1列单元格数据（默认读取sheet1页第1行第1列单元格数据）
    print("\nxlseng.readcell():")
    print xlseng.readcell('sheet1',3,1)
   
    #读取sheet1页第3行的数据（默认读取sheet1页第1行的数据）
    print("\nxlseng.readrow():")
    ...
    #print("\nxlseng.writerow():")
    #xlseng.writerow('rowstr1,rowstr2,rowstr3',0,3,4)
    
    #向第一个sheet页写一列数据，各行的值为‘colstr1,colstr2,colstr3，colstr4’，从第4行第4列开始写入(默认向第一个sheet页写一列数据，值为‘’，从第1行第1列开始写入)
    #print("\nxlseng.writecol():")
    #xlseng.writecol('colstr1,colstr2,colstr3,colstr4',0,4,4)
